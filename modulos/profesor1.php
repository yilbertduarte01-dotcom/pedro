...
// Simulación de sesión si no se ha iniciado sesión por login (Fallback para pruebas)
if (!isset($_SESSION['usuario_logueado'])) {
    $_SESSION['usuario_logueado'] = true;
    $_SESSION['rol'] = 'Profesor';
    $_SESSION['nombre'] = 'Docente de Prueba';
    $_SESSION['username'] = 'jose_paez'; // Nombre de usuario que coincide con la BD
}
...
if ($pdo && !empty($usuario_profesor)) {
    try {
        ...
        // Usamos LOWER y TRIM para evitar fallos por espacios accidentales al registrar en la BD
        // Verificamos en asignaciones_docentes basado en el volcado SQL proporcionado
        $stmt = $pdo->prepare("
            SELECT seccion 
        ...
        // Limpiar espacios en los nombres de las secciones para cruzar correctamente
        ...
        if (!empty($mis_secciones)) {
            foreach ($mis_secciones as $seccion) {
                ...
                // Según tu base de datos, algunos estudiantes tienen seccion 'A' y nivel '2do Año'
                // en lugar de tener seccion '2A'. Debemos desglosar la sección del profesor (Ej: '2A' -> Grado 2, Seccion A)
                
                // Extraer posibles partes de la sección (ej: "2A" -> numero "2", letra "A")
                $numero_grado = preg_replace('/[^0-9]/', '', $seccion); 
                $letra_seccion = preg_replace('/[^A-Za-z]/', '', $seccion);
                
                // Si pudimos separar un número y una letra (ej: 2 y A)
                if (!empty($numero_grado) && !empty($letra_seccion)) {
                     $stmt_est = $pdo->prepare("
                        ...
                        WHERE estado = 'Activo' AND (
                            TRIM(seccion) = ? -- Coincidencia exacta (ej: '2A')
                            OR 
                            (TRIM(seccion) = ? AND nivel_academico LIKE ?) -- Coincidencia separada (ej: seccion 'A' y nivel '%2%')
                        )
                    ...
                    // Búsqueda normal si el formato no es número+letra
                    $stmt_est = $pdo->prepare("
                        SELECT id, cedula, nombre, apellido, email, nivel_academico, seccion 
                        FROM estudiantes 
...
// Variables para UI
$vista_activa = $_GET['seccion'] ?? ($mis_secciones[0] ?? '');
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<body class="flex h-screen overflow-hidden text-slate-800 bg-slate-50">
    ...
    <aside id="sidebar" class="w-72 bg-slate-900 text-white hidden lg:flex flex-col shadow-2xl z-30 transition-all duration-300 absolute lg:relative h-full">
        ...
        <nav class="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto">
            ...
            <div class="text-xs font-semibold text-slate-500 uppercase tracking-wider px-4 pt-4 mb-2">Mis Módulos</div>
                ...
                <?php else: ?>
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-2 overflow-x-auto slide-up">
                        <div class="flex gap-2 min-w-max">
                            ...
                            <?php endforeach; ?>
                            <a href="?seccion=evaluaciones" class="px-5 py-2.5 rounded-xl font-medium text-sm transition-all flex items-center gap-2 <?= ($vista_activa === 'evaluaciones') ? 'bg-amber-500 text-slate-900 shadow-md' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-700' ?> ml-auto">
                    ...
                    <?php if ($vista_activa !== 'evaluaciones'): ?>
                        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden slide-up">
                            <div class="p-6 border-b border-slate-200 bg-slate-50/50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                                ...
                                <div class="px-3 py-1 bg-white border border-slate-200 rounded-lg shadow-sm text-sm font-semibold text-slate-600 flex items-center gap-2">
                            ...
                            <div class="overflow-x-auto">
                                <?php if (empty($estudiantes_por_seccion[$vista_activa])): ?>
                                    <div class="p-10 text-center text-slate-500">
                                        ...
                                        Esta sección no tiene estudiantes registrados actualmente.
                                    </div>
                                <?php else: ?>
                                    <table class="w-full text-left text-sm whitespace-nowrap">
                                        ...
                                        <tbody class="divide-y divide-slate-100 bg-white">
                                            ...
                                            <tr class="hover:bg-slate-50/80 transition-colors group">
                                                <td class="px-6 py-4">
                                                    <div class="flex items-center gap-3">
                                                        <div class="w-9 h-9 rounded-xl bg-slate-100 text-slate-600 flex items-center justify-center font-bold text-xs border border-slate-200 group-hover:bg-amber-100 group-hover:text-amber-600 transition-colors">
                                                ...
                                                <td class="px-6 py-4 text-slate-500 text-xs hidden md:table-cell">
                                                    <span class="bg-slate-100 px-2 py-1 rounded text-slate-600"><?= htmlspecialchars($est['nivel_academico'] ?? 'N/A') ?></span>
                                                    <span class="bg-slate-100 px-2 py-1 rounded text-slate-600 ml-1">Sec: <?= htmlspecialchars($est['seccion']) ?></span>
                                                </td>
                                                <td class="px-6 py-4 text-center">
                                                    <button class="px-3 py-1.5 bg-white border border-slate-200 text-slate-600 hover:text-amber-600 hover:border-amber-300 rounded-lg text-xs font-semibold shadow-sm transition-all">
                    ...
                    <?php else: ?>
                        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden slide-up">
                            <div class="p-6 border-b border-slate-200 bg-slate-50/50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                                ...
                                <button onclick="document.getElementById('modalEvaluacion').classList.remove('hidden'); document.getElementById('modalEvaluacion').classList.add('flex');" class="bg-slate-900 hover:bg-slate-800 text-white font-medium py-2.5 px-5 rounded-xl transition-colors shadow-md flex items-center gap-2 text-sm">
                            ...
                            <div class="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                <?php if (empty($_SESSION['evaluaciones_data'])): ?>
                                    <div class="col-span-full p-10 text-center text-slate-500 border-2 border-dashed border-slate-200 rounded-2xl">
                                        <i class="fa-solid fa-clipboard-list text-4xl text-slate-300 mb-3 block"></i>
                                        Aún no has planificado ninguna evaluación.
                                    </div>
                                <?php else: ?>
                                    <?php foreach ($_SESSION['evaluaciones_data'] as $ev): ?>
                                        <div class="border border-slate-200 rounded-xl p-5 hover:border-amber-400 hover:shadow-md transition-all relative">
                                            <span class="absolute top-4 right-4 text-xs font-bold px-2 py-1 bg-amber-50 text-amber-600 rounded-md border border-amber-200">
                                            ...
                                            <h4 class="text-lg font-bold text-slate-800 mb-2 leading-tight"><?= htmlspecialchars($ev['titulo']) ?></h4>
                                            <div class="flex items-center gap-2 text-sm text-slate-500 mb-4">
                                                <i class="fa-regular fa-calendar"></i> <?= date('d/m/Y', strtotime($ev['fecha'])) ?>
                                            </div>
                                            <div class="text-sm font-medium text-slate-600 bg-slate-50 inline-block px-3 py-1 rounded-lg border border-slate-100">
    ...
    <div id="modalEvaluacion" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm hidden items-center justify-center z-50 px-4">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-white/20 slide-up">
            <div class="flex justify-between items-center p-6 border-b border-slate-100">
                <h2 class="text-xl font-bold text-slate-800">Programar Evaluación</h2>
                <button onclick="document.getElementById('modalEvaluacion').classList.add('hidden'); document.getElementById('modalEvaluacion').classList.remove('flex');" class="text-slate-400 hover:text-red-500 w-8 h-8 rounded-lg flex items-center justify-center transition-colors">
            ...
            <form method="POST" class="p-6 space-y-4">
                <div>
                    ...
                    <input type="text" name="titulo" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-amber-500 bg-slate-50 focus:bg-white transition-all outline-none" required placeholder="Ej: Examen de Matemáticas">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1">Sección Destino</label>
                    <select name="seccion" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-amber-500 bg-slate-50 focus:bg-white transition-all outline-none" required>
                        <?php foreach($mis_secciones as $sec): ?>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="tipo" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-amber-500 bg-slate-50 outline-none" required>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">Ponderación (%)</label>
                        <input type="number" name="ponderacion" min="1" max="100" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-amber-500 bg-slate-50 outline-none" required placeholder="20">
                    </div>
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1">Fecha de Ejecución</label>
                    <input type="date" name="fecha" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-amber-500 bg-slate-50 outline-none" required>
                </div>
                <button type="submit" name="guardar_evaluacion" class="w-full bg-slate-900 hover:bg-slate-800 text-white font-semibold py-3 rounded-xl transition-all shadow-md mt-2">
                    Guardar Actividad
                </button>
            </form>
        </div>
