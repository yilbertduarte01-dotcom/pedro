...
try {
    ...
    // Extraer estudiantes de la base de datos para el select
...
<body class="flex h-screen overflow-hidden text-slate-800 bg-slate-50">
    ...
    <!-- Modal: Asignar Pasantía (Con conexión a BD) -->
        ...
        <div class="modal-content">
            ...
            <form method="POST">
                <div class="form-group">
                    ...
                    <!-- REEMPLAZO AQUI: Se usa un select que lee de la base de datos MySQL -->
                    <select name="estudiante_nombre" class="form-input" required>
                        <option value="">Seleccione un estudiante de la base de datos...</option>
                        <?php foreach($estudiantes_db as $est): ?>
                            <?php $nombre_completo = trim($est['nombre'] . ' ' . $est['apellido']); ?>
                            <option value="<?php echo htmlspecialchars($nombre_completo); ?>">
                                <?php echo htmlspecialchars($nombre_completo . ' (C.I: ' . $est['cedula'] . ' - Sec: ' . ($est['seccion'] ?? 'N/A') . ')'); ?>
                            </option>
                        <?php endforeach; ?>
